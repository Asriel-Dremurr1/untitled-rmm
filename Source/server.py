...
def verify_basic_auth():
    """Проверка аутентификации с поддержкой первого входа для Admin"""
    ...
    try:
        ...
        # Специальная обработка для Admin с пустым или отсутствующим паролем
        if username == "Admin":
            # Если запись Admin существует, но пароль пустой
            if user and (not user.get('password_hash') or user.get('password_hash') == ""):
                # Принимаем любой пароль при первом входе
                # Сохраняем введенный пароль как хэш
                if len(password) >= 8:
                    users["Admin"]["password_hash"] = generate_password_hash(password)
                    save_users()
                    print(f"[*] Admin password set on first login")
                    return username
                else:
                    # Слишком короткий пароль - все равно пускаем, но предупреждаем в лог
                    users["Admin"]["password_hash"] = generate_password_hash(password)
                    save_users()
                    print(f"[!] WARNING: Admin password set with {len(password)} chars (less than 8)")
                    return username
            
            # Если запись Admin отсутствует в users (создаем её)
            if not user:
                # Создаем Admin с введенным паролем
                if len(password) >= 8:
                    users["Admin"] = {
                    ...
                    save_users()
                    print(f"[*] Admin user created with password on first login")
                    return username
                else:
                    # Слишком короткий пароль - все равно создаем
                    users["Admin"] = {
                        "password_hash": generate_password_hash(password),
                        "privileges": PRIVS,
                        "cmd_blacklist": [],
                    ...
                    save_users()
                    print(f"[!] WARNING: Admin created with {len(password)} chars password")
                    return username
